...
2. 最终回复"已完成编辑"，不要在回复中输出文件内容`;
  ...
  private getStandardToolRules(): string {
    return `1. 当你需要读取或写入文件时，直接调用工具，不要询问用户确认。
2. 不要在回复中输出工具调用的代码或标签（如 <tool_call>），直接使用工具即可。
3. 工具执行完成后，用一两句话简要告知结果，不要输出整理过程。
4. 回复要简洁，不要重复笔记内容。
...
2. 根据用户需求修改内容
3. 立即调用 write_file 写入修改后的完整内容
4. 最终回复只说"已完成编辑"或类似简短确认，不要在回复中输出文件内容

重要：不要在文本回复中输出完整的文件内容，直接调用 write_file 写入即可。`;
  }
  ...
  private async sendMessage(): Promise<void> {
    ...
    if (note) {
      fullMessage = `${content}\n\n<current_note>\npath: ${note.path}\ncontent:\n${note.content}\n</current_note>`;
      // 从笔记中提取图片（仅 Ollama 视觉模型需要）
      if (this.currentProvider === "ollama") {
        noteImages = await this.extractImagesFromNote(note.path, note.content);
        if (noteImages.length > 0) {
          console.log(`[AI Lexi] 从笔记中提取 ${noteImages.length} 张图片`);
        }
      }
    }
    ...
    try {
      // 发送请求（支持工具调用）
      this.updateLoadingText(loadingEl, `正在向 ${this.getProviderName()} 发送请求...`);
      let response = await this.sendRequestWithTools();

      // 如果有工具调用，循环处理直到没有工具调用
      let loopCount = 0;
      while (response.success && response.toolCalls && response.toolCalls.length > 0) {
        ...
        for (const tc of response.toolCalls) {
          this.addStatusMessage(`🔧 调用工具: ${tc.function.name}`);
        }

        // 将 assistant 的 tool_calls 加入对话历史（DeepSeek/Xiaomi 要求 tool_call_id 对应）
        // Ollama 原生 API 不支持 OpenAI 格式的 tool_calls，跳过
        if (this.currentProvider !== "ollama") {
          this.conversation.addAssistantMessageWithToolCalls(response.content || "", response.toolCalls);
        }
        ...
        // Ollama：成功执行 write_file 后直接结束，不再继续调 API
        // 否则模型会反复输出 <tool_call> 导致循环
        if (this.currentProvider === "ollama" && response.toolCalls.some((tc: any) => tc.function.name === "write_file")) {
          break;
        }

        // 继续对话，让 AI 看到工具结果
        this.updateLoadingText(loadingEl, `等待 ${this.getProviderName()} 回复...`);
        response = await this.sendRequestWithTools();
      }
      ...
      if (response.success) {
        if (response.toolCalls && response.toolCalls.length > 0) {
          ...
          // 没有工具调用时显示文本内容
          const displayContent = this.stripToolCallText(response.content);
          if (displayContent) {
            this.addMessage("assistant", displayContent);
            this.conversation.addAssistantMessage(displayContent);
          } else {
      ...
      if (err.name === "AbortError") {
        ...
        this.addErrorMessage(`❌ 请求异常: ${err.message || "未知错误"}`);
  ...
  private async handleToolCalls(toolCalls: any[]): Promise<void> {
    for (const toolCall of toolCalls) {
      ...
      const action = name === "read_file" ? "Read" : "Write";
      if (!args || !args.path) {
        console.warn("[AI Lexi] 工具调用参数缺少 path 字段，跳过:", name, JSON.stringify(args));
        continue;
      }
      ...
      const content = name === "read_file" && !result.includes("错误")
        ...
        : result;
      this.conversation.addToolResult(name, content, toolCall.id);
    }
  }
